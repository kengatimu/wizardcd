...
# Paths
# --------------------------------------------------
# Resolve configuration file path (supports ~/.wizardcd/ or local config/)
...
APP_JAR_NAME=$(yq -r ".apps.${APP}.${ENV}.build_paths.jar" "$CONFIG_FILE")
APP_LIB_DIR_NAME=$(yq -r ".apps.${APP}.${ENV}.build_paths.lib" "$CONFIG_FILE")
APP_JAR=$(yq -r ".apps.${APP}.${ENV}.app_jar" "$CONFIG_FILE") # Name JAR should have on server
...
# --------------------------------------------------
# These files are part of the core WizardCd installation and
# are not user-supplied. This ensures consistent packaging
# across Brew, Deb, and RPM environments.
...
# --------------------------------------------------
# Load optional paths from YAML (simple and portable)
# --------------------------------------------------
OPTIONAL_PATHS=$(yq -r ".apps.${APP}.${ENV}.optional_paths[]" "$CONFIG_FILE" 2>/dev/null || echo "")

# --------------------------------------------------
# Resolve artifact paths using project_base_path
# --------------------------------------------------
# Purpose:
#   - This ensures WizardCd works seamlessly across local, VM, and CI/CD environments.
#   - The JAR and LIB paths are derived from the absolute project_base_path.
#   - Removes the need for discovery or relative path normalization.
#   - Faster, predictable, and portable.
...
if [[ -z "$APP_LIB_SRC" || ! -d "$APP_LIB_SRC" ]]; then
...
# --------------------------------------------------
# Copy dependency libs (optional)
# --------------------------------------------------
if [[ -n "$APP_LIB_SRC" && -d "$APP_LIB_SRC" ]]; then
  log_info "Copying dependency libs from $APP_LIB_SRC ..."
  mkdir -p "$PKG_DIR/bin/lib"                               # Ensure bin/lib exists for dependencies
  shopt -s nullglob
  files=("$APP_LIB_SRC"/*)
  shopt -u nullglob
  if [[ ${#files[@]} -gt 0 ]]; then
    cp -r "$APP_LIB_SRC/"* "$PKG_DIR/bin/lib/"              # Copy all dependency jars under bin/lib/
  else
    log_warn "Dependency lib directory is empty (skipping copy)."
  fi
else
  log_warn "No dependency lib directory found (skipping)."
fi
